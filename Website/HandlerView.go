...
// This HTTP handler shows all of the study groups in the collection.
func HandlerView(response http.ResponseWriter, request *http.Request) {
	...
	if errFind := collGroups.Find(selection).All(&templateData.Groups); errFind != nil {
	...
	Templates.ProcessHTML(`ViewGroups`, response, templateData)
}
